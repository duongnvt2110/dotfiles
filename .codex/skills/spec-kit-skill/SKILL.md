...
- Primary role: greenfield constitution-first spec workflow.
- Must trigger for explicit Spec-Kit intent (`spec-kit`, `speckit`, `constitution`, `.specify`, `specify` commands).
- Must yield to `openspec` for brownfield existing-system spec changes.
- Must yield to `planner` for generic planning prompts without spec-kit signals.
- Handoff rule: after spec-kit artifacts are produced, hand off execution decomposition to `planner` or project task workflow.
