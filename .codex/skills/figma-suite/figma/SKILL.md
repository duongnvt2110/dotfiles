...
- Primary role: default non-cache Figma workflow for generic MCP usage.
- Must yield to `figma-cache` when user explicitly requests cache-first behavior or reliability fallback.
- Must yield to `figma-implement-design` or `figma-implement-design-cache` when the task is explicit production implementation from Figma.
- Must yield to `figma-detail-extractor` when the requested output is detailed specs/layer tables/assets documentation.
- Handoff rule: if implementation or detailed extraction becomes the objective, hand off to the specialized skill.
