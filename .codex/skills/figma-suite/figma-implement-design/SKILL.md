...
- Primary role: non-cache production implementation from Figma.
- Must trigger for explicit implementation intent (build/implement/component/page from Figma).
- Must yield to `figma-implement-design-cache` when cache-first implementation is explicitly requested.
- Must yield to `figma-detail-extractor` when the output required is a detailed extraction report rather than code implementation.
- Must not trigger for generic Figma troubleshooting/setup/inspection; defer to `figma`.
