...
- Primary role: cache-first production implementation from Figma.
- Must trigger for explicit implementation intent combined with cache-first or reliability preference.
- Must yield to `figma-detail-extractor` when the requested deliverable is extraction/spec documentation rather than code implementation.
- Must not trigger for generic Figma tasks; defer to `figma` or `figma-cache` based on cache intent.
